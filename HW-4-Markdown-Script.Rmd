...
```{r}
nutrients <- read.csv("/Users/dariabrown/Documents/GitHub/189/Math-189-Projects/Homework_4/nutrients.csv")
...
```{r}
multiple <- read.table("/Users/dariabrown/Documents/GitHub/189/Math-189-Projects/Homework_4/multiple.txt", quote="\"", comment.char="")
```

### Part A
Ho: u = 0 vs. u =! 0 
We need to perform a two-sided t-test.

```{r}
m = 50
pv <- numeric(m)
for(j in 1:m){
  t_test <- t.test(multiple[,j], alternative="two.sided", mu=0,conf.level = 0.9) 
  pv[j] <- t_test$p.value
}
```
...
```{r}
sum(pv[1:10] > 0.1) # false negatives (We want the number of times we kept the null, and we should have rejected - Type 2 Error)
sum(pv[11:50] < 0.1) # false positive (We want number of times we rejected null when we should have kept -type 1 error!)
sum(pv[11:50] < 0.1)/sum(pv[1:50] < 0.1) # false discovery proportion (Type 1 Error / Total number of rejections of null)
```
### Part C

```{r}
bonf <- p.adjust(pv,method="bonferroni")
sum(bonf[1:10] > 0.1) # Type 2 Error
sum(bonf[11:50] < 0.1) # Type 1 Error
